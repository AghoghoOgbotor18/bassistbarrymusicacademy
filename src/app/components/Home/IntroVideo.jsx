            ...
            <div className="max-w-6xl mx-auto">
                <div className="grid lg:grid-cols-2 gap-12 items-center">
                    ...
                    <div className="relative">
                        ...
                        <div className="relative rounded-2xl overflow-hidden aspect-video shadow-2xl bg-ebony">
                            ...
                            {showControls && (
                                <div className={`
                                    ...
                                    bg-gradient-to-t from-ebony/70 via-transparent to-transparent
                                ...
                                `}>
                                    ...
                                    <button
                                        onClick={togglePlay}
                                        className="w-10 h-10 rounded-full bg-maple flex items-center justify-center hover:bg-maple/90 active:scale-95 transition"
                                        aria-label={isPlaying ? "Pause" : "Play"}
                                    >
                                        {isPlaying ? (
                                            <svg className="w-4 h-4 fill-current text-ebony" viewBox="0 0 24 24">
                                                <rect x="6" y="4" width="4" height="16" />
                                                <rect x="14" y="4" width="4" height="16" />
                                            </svg>
                                        ) : (
                                            <svg className="w-4 h-4 fill-current text-ebony ml-0.5" viewBox="0 0 24 24">
                                                <polygon points="5,3 19,12 5,21" />
                                            </svg>
                                        )}
                                    </button>

                                    {/* Mute/Unmute — prominent on mobile */}
                                    <button
                                        onClick={toggleMute}
                                        className={`
                                            flex items-center gap-2 text-xs px-4 py-2 rounded-full
                                            transition active:scale-95 font-medium
                                            ${isMuted
                                                ? "bg-maple text-ebony animate-pulse"
                                                : "bg-ebony/70 text-parchment"
                                            }
                                        `}
                            ...
                            {/* Initial state — before video starts */}
                            {!hasPlayed && (
                                <div className="absolute inset-0 flex items-center justify-center bg-ebony/30">
                                    <div className="flex flex-col items-center gap-3">
                                        <div className="w-16 h-16 rounded-full bg-maple flex items-center justify-center animate-pulse">
                                            <svg className="w-6 h-6 fill-current text-ebony ml-1" viewBox="0 0 24 24">
                                                <polygon points="5,3 19,12 5,21" />
                                            </svg>
                                        </div>
                                        <p className="text-parchment text-xs font-mono">
                                            Loading video...
                                        </p>
                                    </div>
                                </div>
                        ...
                        {/* Muted hint below video — mobile only */}
                        {isMuted && isPlaying && (
